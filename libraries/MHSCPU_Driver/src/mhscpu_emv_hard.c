...
static  const uint8_t        aSCI_CG_Table[3] = {(uint8_t)SCI0_CG_BIT};
volatile struct emv_queue  scs_queue[3]; /* datas buffer */
...
int emv_hard_power_pump( struct emv_core *pdev )
{
    ...
    //Set working voltage.
    emv_hard_VoltageSelect(u32Tmp & 0x03);
    if (SCICARDDN != (aSCI_BaseTable[pdev->terminal_ch]->SCI_IIR & SCICARDDN))
    {
...
void emv_hard_SetCardNormalGlf(int clock)
{
    ...
    {
        SYSCTRL->SCI_GLF = (SYSCTRL->SCI_GLF & ~0xFFFFF) | (clock & 0xFFFFF);
    }
}

void emv_hard_VoltageSelect(SCI_VoltageTypeDef SCI_Voltage)
{
    assert_param(IS_SCI_VOLTAGE(SCI_Voltage));
    
    SYSCTRL->SCI_GLF = (SYSCTRL->SCI_GLF & ~(0x3 << 28)) | (SCI_Voltage << 28);
}

void emv_hard_CardDetectSelect(unsigned char SCI_Bitmap, SCI_CardDetectTypeDef CardDetect)
{
    assert_param(IS_SCI_CARDDETECT(CardDetect));
    
    switch(SCI_Bitmap)
    {
    case BIT(0):
        SYSCTRL->PHER_CTRL = (SYSCTRL->PHER_CTRL & ~BIT(16)) | (CardDetect << 16);
        break;
    }
}
...
{
    assert_param(IS_SCI_VCCEN(VccEn));
    
    switch(SCI_Bitmap)
    {
    case BIT(0):
        SYSCTRL->PHER_CTRL = (SYSCTRL->PHER_CTRL & ~BIT(20)) | (VccEn << 20);
        break;
    }
}
